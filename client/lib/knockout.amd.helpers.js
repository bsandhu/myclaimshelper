...
//an AMD template engine that uses the text plugin to pull templates
    (function (ko, require) {
        ...
        ko.templateSources.rawTemplate = function (content) {
            this.content = content;
        };

        ko.templateSources.rawTemplate.prototype.text = function (value) {
            if (arguments.length === 0) {
                return this.content;
            } else {
                throw "Not expecting arsg to text()";
            }
        };
        ...
        engine.makeTemplateSource = function (template, doc) {
            var el;
            var errMsg =
                "Please pass actual template content to the `name` attr of the `template` binding. " +
                "Binding to fix: " + template;

            //if a name is specified
            if (typeof template === "string") {
                // If we are passing in actual template content, just return
                if (template.indexOf('<') >= 0 && template.indexOf('>') >= 0) {
                    return new ko.templateSources.rawTemplate(template);
                }
