...
function serveStaticWith304(opts) {
    opts = opts || {};
    ...
    function serveFileFromStats(file, err, stats, isGzip, req, res, next) {
        ...
        fstream.once('open', function (fd) {
            ...
            // Note:
            // Forked from restify > conditional_request since serveStatic does not handle
            // conditional requests with 304 return code
            //
            // ***** Conditional response *****
            var cacheTime = req.header('if-modified-since');
            var modTime = res.header('Last-Modified');
            var code = undefined;
            if (modTime && cacheTime) {
                try {
                    if (Date.parse(modTime) <= Date.parse(cacheTime)) {
                        switch (req.method) {
                            case 'GET':
                            case 'HEAD':
                                code = 304;
...
module.exports = serveStaticWith304;
